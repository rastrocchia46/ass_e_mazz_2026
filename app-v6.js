(() => {
  ...
  function buildOttaviRows(standings) {
    ...
    return seedTables.flatMap((seeds, tableIndex) => seeds.map((seed, seatIndex) => {
  ...
  function buildSnakeFinalRows(qualifiedRows, tableCount) {
    const sorted = [...qualifiedRows].sort(compareFinalPerformance);
    const seats = new Array(tableCount).fill(0);
    const rows = sorted.map((player, index) => {
      const cycleLength = tableCount * 2;
      const cyclePosition = index % cycleLength;
      const tableIndex = cyclePosition < tableCount
        ? cyclePosition
        : cycleLength - 1 - cyclePosition;
      seats[tableIndex] += 1;
      return {
        table: String.fromCharCode(65 + tableIndex),
        seat: seats[tableIndex],
        seed: player.seed,
    ...
    return rows.sort((a, b) => a.table.localeCompare(b.table, "it") || a.seat - b.seat);
  }
  ...
  function getFinalsData() {
    ...
    const ottaviQualifiers = ottavi.complete ? getFinalQualifiers("ottavi", ottavi.rows) : [];
    if (!quartiRows.length && ottaviQualifiers.length === 15) {
      quartiRows = buildSnakeFinalRows(ottaviQualifiers, 3);
    ...
    const quartiQualifiers = quarti.complete ? getFinalQualifiers("quarti", quarti.rows) : [];
    if (!semifinaliRows.length && quartiQualifiers.length === 10) {
      semifinaliRows = buildSnakeFinalRows(quartiQualifiers, 2);
    ...
    const semifinaliQualifiers = semifinali.complete ? getFinalQualifiers("semifinali", semifinali.rows) : [];
    if (!finaleRows.length && semifinaliQualifiers.length === 5) {
      finaleRows = buildSnakeFinalRows(semifinaliQualifiers, 1);
  ...
  function renderFinals() {
    ...
    const note = $("#final-stage-note");
    note.className = "final-stage-note";
    if (!finals.eliminationComplete) {
      note.classList.add("waiting");
      note.textContent = "Gli accoppiamenti saranno disponibili al termine della fase eliminatoria.";
    } else if (stage.source === "preview") {
      note.classList.add("preview");
      note.textContent = state.finalStage === "ottavi"
        ? "Accoppiamenti determinati dalla classifica ufficiale: 1° con 25°, 2° con 24° e così via. I primi cinque sono distribuiti in tavoli diversi."
        : "Anteprima degli accoppiamenti calcolata sui qualificati della fase precedente.";
    } else if (!stage.rows.length) {
      note.classList.add("waiting");
      note.textContent = "Questa fase sarà composta automaticamente quando saranno disponibili tutti i qualificati.";
    } else {
      note.textContent = state.finalStage === "quarti"
        ? "Passano i primi tre di ogni tavolo e il miglior quarto classificato."
        : state.finalStage === "semifinali"
          ? "Passano i primi due di ogni tavolo e il miglior terzo classificato."
          : state.finalStage === "finale"
            ? "Tavolo unico da cinque giocatori per decretare il vincitore assoluto."
            : "Passano i primi tre classificati di ogni tavolo.";
    }
          ...
          <div class="table-players">
            ${orderedRows.map((row) => {
              ...
              const details = completed
                ? `${row.gamePoints ?? "—"} punti`
                : `<span class="seed-label">Seed ${row.seed || "—"}</span>Sedia ${row.seat}`;
              return `
                <div class="table-player ${row.position === 1 ? "winner" : ""} ${advanced ? "advanced" : ""}">
                  ...
                  <div class="table-player-result"><strong>${completed ? `${row.gamePoints ?? "—"}` : "—"}</strong><small>punti partita</small></div>
                </div>`;
            }).join("")}
          </div>
        </article>`;
