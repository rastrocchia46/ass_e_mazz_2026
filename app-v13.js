(() => {
  ...
  const FINAL_STAGE_META = {
    // Chiavi interne mantenute per compatibilità con il layout esistente.
    // ottavi = Turno 1, quarti = Turno 2, semifinali = Turno 3.
    ottavi: { label: "Turno 1", short: "T1", tableCount: 5, playerCount: 25, topEach: 3, wildcardPlace: null, wildcardCount: 0 },
    quarti: { label: "Turno 2", short: "T2", tableCount: 4, playerCount: 20, topEach: 2, wildcardPlace: 3, wildcardCount: 2 },
    semifinali: { label: "Turno 3", short: "T3", tableCount: 2, playerCount: 10, topEach: 2, wildcardPlace: 3, wildcardCount: 1 },
  ...
  function rowsToTournamentData(configRows, playersRows, resultRows, source = "excel") {
    ...
    const playersCount = toNumber(config["numero giocatori"]) || 40;
    ...
    return {
      source,
      config: {
        title: String(config["nome torneo"] || "Torneo Ass 'e Mazz 2026 - II Edizione"),
        playersCount,
        cutoff: toNumber(config["posti qualificazione"]) || 30,
  ...
  function workbookFinalRows(workbook, sheetName, finalTableName = null) {
    const rows = sheetRows(workbook, sheetName);
    const headerIndex = locateHeader(rows, ["Giocatore ID", "Punti partita"]);
    const objects = rowsToObjects(rows, headerIndex);
    const parsed = objects.map((row) => ({
      table: String(finalTableName || row.tavolo || "").trim().toUpperCase(),
      seat: toNumber(row.sedia),
      seed: toNumber(row["seed iniziale"] || row["seed t2"] || row["seed t3"] || row["ordine accesso"]),
      playerId: toNumber(row["giocatore id"]),
      gamePoints: toNumber(row["punti partita"]),
      position: toNumber(row.posizione || row["posizione finale"]),
      sole: toNumber(row.sole) || 0,
      note: String(row.note || "").trim(),
    })).filter((row) => row.table && row.playerId);
    const expected = { Turno_1: 25, Turno_2: 20, Turno_3: 10, Finale: 5 }[sheetName] || 0;
    return expected && parsed.length < expected ? [] : parsed;
  }
  ...
  function workbookToData(workbook) {
    ...
    const data = rowsToTournamentData(configRows, playersRows, resultRows, "excel");
    data.finals = {
      ottavi: { source: "excel", rows: workbookFinalRows(workbook, "Turno_1") },
      quarti: { source: "excel", rows: workbookFinalRows(workbook, "Turno_2") },
      semifinali: { source: "excel", rows: workbookFinalRows(workbook, "Turno_3") },
      finale: { source: "excel", rows: workbookFinalRows(workbook, "Finale", "FINALISSIMA") },
    };
    return data;
  }
  ...
  function buildOttaviRows(standings) {
    const bySeed = new Map(standings.slice(0, 30).map((entry, index) => [index + 1, { ...entry, seed: index + 1 }]));
    const seedTables = [
      { table: "A", seeds: [6, 15, 16, 25, 26] },
      { table: "B", seeds: [7, 14, 17, 24, 27] },
      { table: "C", seeds: [8, 13, 18, 23, 28] },
      { table: "D", seeds: [9, 12, 19, 22, 29] },
      { table: "E", seeds: [10, 11, 20, 21, 30] },
    ];
  ...
  function buildQuartiRows(turno1Rows, standings) {
    const direct = standings.slice(0, 5).map((entry, index) => ({
      playerId: entry.id,
      seed: index + 1,
      position: 0,
      gamePoints: null,
    ...
    const turno1Qualified = getFinalQualifiers("ottavi", turno1Rows)
      .sort(compareFinalPerformance);
    const pool = direct.concat(turno1Qualified).map((player, index) => [index + 1, player]);
    const bySeedT2 = new Map(pool);
    const structure = [
      { table: "A", seeds: [1, 8, 9, 16, 17] },
      { table: "B", seeds: [2, 7, 10, 15, 18] },
      { table: "C", seeds: [3, 6, 11, 14, 19] },
      { table: "D", seeds: [4, 5, 12, 13, 20] },
    ];
    return structure.flatMap((group) => group.seeds.map((seed, index) =>
      makePreviewFinalRow(group.table, index + 1, bySeedT2.get(seed))))
      .filter((row) => row.playerId);
  }

  function buildSemifinaliRows(turno2Rows) {
    const qualified = getFinalQualifiers("quarti", turno2Rows).sort(compareFinalPerformance);
    const bySeedT3 = new Map(qualified.map((player, index) => [index + 1, player]));
    const structure = [
      { table: "A", seeds: [1, 4, 5, 8, 9] },
      { table: "B", seeds: [2, 3, 6, 7, 10] },
    ];
    return structure.flatMap((group) => group.seeds.map((seed, index) =>
      makePreviewFinalRow(group.table, index + 1, bySeedT3.get(seed))))
      .filter((row) => row.playerId);
  }

  function buildFinaleRows(turno3Rows) {
    const finalists = getFinalQualifiers("semifinali", turno3Rows).sort(compareFinalPerformance);
  ...
  function getFinalsData() {
    ...
    if (!quartiRows.length && ottavi.complete) {
      quartiRows = buildQuartiRows(ottavi.rows, standings);
  ...
  function renderFinalsBoard() {
    ...
    const stageColumns = { ottavi: 1, quarti: 3, semifinali: 5, finale: 7 };
    const stageRows = {
      // Ogni tavolo occupa tre righe della griglia. Gli intervalli più ampi
      // impediscono la sovrapposizione dei cinque tavoli degli ottavi.
      ottavi: [2, 5, 8, 11, 14],
      quarti: [2, 6, 10, 14],
      semifinali: [5, 11],
      finale: [8],
    ...
    const stageHeadings = FINAL_STAGE_ORDER.map((stageKey) => {
      const stage = finals[stageKey];
      const status = finalStageStatus(stage);
      return `
        <div class="tournament-stage-heading ${stage.complete ? "complete" : ""}" style="--column:${stageColumns[stageKey]}">
          <strong>${stage.label}</strong>
          <small>${stage.playerCount} giocatori · ${stage.tableCount} ${stage.tableCount === 1 ? "tavolo" : "tavoli"}</small>
          <span class="bracket-status ${status.className}">${status.label}</span>
        </div>`;
    }).join("");

    const rails = `
      <span class="tournament-rail rail-ottavi-quarti" aria-hidden="true"></span>
      <span class="tournament-rail rail-quarti-semifinali" aria-hidden="true"></span>
      <span class="tournament-rail rail-semifinali-finale" aria-hidden="true"></span>`;

    const matches = FINAL_STAGE_ORDER.flatMap((stageKey) => {
      const stage = finals[stageKey];
      ...
      if (!groups.length) {
        return [`
          <article class="tournament-match placeholder-match stage-${stageKey}" style="--column:${stageColumns[stageKey]};--row:${rows[0]}">
            <header><strong>${stageKey === "finale" ? "Finalissima" : "Da definire"}</strong></header>
            <div class="tournament-placeholder">Partecipanti da definire</div>
          </article>`];
      }
      ...
      return groups.map((group, index) => {
        ...
        const winnerId = stageKey === "finale" ? finals.winnerPlayerId : null;

        return `
          <article class="tournament-match stage-${stageKey} ${group.completed ? "complete" : ""} ${current ? "current" : ""}" style="--column:${stageColumns[stageKey]};--row:${rows[index] || rows.at(-1)}">
            <header>
              <strong>Tavolo ${escapeHtml(group.table)}</strong>
              <span class="bracket-status ${status.className}">${group.completed ? "Concluso" : group.started ? "In corso" : "Da giocare"}</span>
            </header>
            <div class="tournament-player-list">
              ${orderedRows.map((row) => {
                const player = playerMap.get(row.playerId);
                const advanced = qualifiedIds.has(row.playerId);
                const isWinner = winnerId === row.playerId && row.position === 1;
                const result = row.position ? `${row.position}°` : "";
                const score = row.gamePoints !== null ? `<em>${row.gamePoints}</em>` : "";
                const badge = isWinner
                  ? '<small class="tournament-badge winner">Vincitore</small>'
                  : advanced
                    ? '<small class="tournament-badge">Avanza</small>'
                    : "";
                return `
                  <div class="tournament-player ${advanced ? "advanced" : ""} ${isWinner ? "champion" : ""}">
                    <i>${result || "•"}</i>
                    <span><b>${escapeHtml(player?.name || `Giocatore ${row.playerId}`)}</b>${badge}</span>
                    ${score}
                  </div>`;
              }).join("")}
            </div>
    ...
    board.innerHTML = `
      <div class="tournament-bracket" aria-label="Tabellone delle fasi finali">
        ${stageHeadings}
        ${rails}
        ${matches}
      </div>`;
        ...
        <article class="table-card">
          ...
          <div class="table-players">
            ${orderedRows.map((row) => {
              ...
              const advanced = qualifiedIds.has(row.playerId);
              const isChampion = state.finalStage === "finale"
                && finals.winnerPlayerId === row.playerId
                && row.position === 1;
              const positionBadge = completed
                ? `<span class="seat-or-place">${row.position}°</span>`
                : "";
              const advancementBadge = isChampion
                ? '<span class="advance-badge winner-badge">Vincitore</span>'
                : advanced
                  ? '<span class="advance-badge">Qualificato</span>'
                  : "";
              return `
                <div class="table-player final-player ${completed ? "" : "final-player-pending"} ${row.position === 1 ? "winner" : ""} ${advanced ? "advanced" : ""} ${isChampion ? "champion" : ""}">
                  ${positionBadge}
  ...
  function renderDashboard() {
    ...
    $("#hero-player-count").textContent = `${config.playersCount} giocatori`;
    $("#ranking-cutoff-bubble").textContent = usingFinals ? `${config.cutoff} qualificati` : `Top ${config.cutoff}`;
    ...
    if (rulePoints) rulePoints.textContent = usingFinals ? "101" : "81";
    ...
    $("#ranking-panel-title").textContent = usingFinals ? "30 qualificati alla fase finale" : "Prime posizioni";
